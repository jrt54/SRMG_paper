...
\begin{figure}[H]
% \minipage{0.48\textwidth}
%   \includegraphics[width=\linewidth]{coarsevsn.png}
...
\draw[blue,thick,dashed] (0,0) -- (2.25,0) -- (2.25,2.25) -- (0,2.25) -- cycle;%[0,2]^2
\node[draw,align=center, blue] at (1,1) {$\Omega_1^{(1)} $};

\draw[red,thick,dashed] (4,0) -- (1.75,0) -- (1.75,2.25) -- (4,2.25) -- cycle;%[2, 4] x [0, 2] 
\node[draw,align=center, red] at (3,1) {$\Omega_2^{(1)} $};
\draw[green,thick,dashed] (4,4) -- (1.75,4) -- (1.75,1.75) -- (4,1.75) -- cycle;%[2, 4] x [2, 4]
\node[draw,align=center, green] at (3,3) {$\Omega_4^{(1)} $};
\draw[orange,thick,dashed] (0,4) -- (2.25,4) -- (2.25, 1.75) -- (0,1.75) -- cycle; %[2, 4] x [2, 4]
\node[draw,align=center, orange] at (1,3) {$\Omega_3^{(1)} $};
\end{tikzpicture}
...
%Recall that according to our corollary \ref{corollaryunique} $u^{(0)}$ restricted to $\Omega^{(1)}_i$ is a solution to the above equation and is thus the unique solution. 
...
For simplicity, consider Poisson's equation $\bigtriangleup u = f$ with zero boundary value conditions on the full domain, so that any approximation or patch encodes the true boundary values where its domain  intersects with the boundary of the full domain. %In practice, we could replicate this phenomenon by decoupling the conditions. 
...
%The coarse error has a simple expression $\tilde{e}^{(0)} = \tilde{u}^{(0)} - u = \tilde{e}^{0}$


%\subsection{Recursive Error Analysis}
