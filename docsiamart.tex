...
\title{Analysis of Segmental Refinement Multigrid} 
\author{Jeremy Tillay%
  \thanks{Computational and Applied Mathematics, Rice University, Houston, TX (\email{jrt5493@gmail.com}).} 
  \and 
  {Matthew Knepley%
  \thanks{Department of Computer Science and Engineering, University at Buffalo}
  } %
}
...
\headers{Analysis of Segmental Refinement Multigrid}{Jeremy Tillay, Matthew Knepley}
...
%   \end{center}%
%   \caption{\footnotesize{Significant error is constrainted to triangular black points}}
% \end{wrapfigure}


\begin{wrapfigure}{r}{0.4\textwidth}
  \begin{center}
    \includegraphics[scale=0.4]{errorsrmg.png}
  \end{center}
  \caption{\footnotesize{Significant error (\textcolor{red}{red}) decays as we move from the boundary }}
\end{wrapfigure}
	...
	There are two unanswered questions regarding SRMG. One is whether we have convergence guarantees in spite of communication no longer occuring between patches.  We show error induced by no longer communicating with the entire domain is localized and does not extend past the buffer regions of the domain decomposition. 
    
	The second question is whether SRMG converges on an arbitrary number of refinement levels. We demonstrate a set of requirements which, when satisfied, guarantee convergence on an arbitrary number of levels. We accompanied my analysis by implementing the method with a spectral solver, to investigate the feasibility of these requirements. 
    %but I demonstrate that if one is careful to account for modest param
    

\section{Background}\label{sec:backg}
%By 1982, Multigrid was an accepted numerical algorithm for solving elliptic equations \cite{earlymulti}. 

The speed at which multigrid solves large problems is one of its primary strengths. One of the problems with multigrid, as well as many other numerical algorithms, is that memory requirements scales linearly. Implementing multigrid on modern large-scale computer architectures also imposes communication requirements which can negatively impact its speed. SRMG was a proposed modification to multigrid that would remedy these issues. 
   
    Models of the SRMG algorithm were presented in detail as early as 1994 \cite{brandtdiskin}. However, there is still debate over what conditions must be met for the algorithm to guarauntee  multi-level convergence. One question that had been left unanswered by previous research is whether Segmental Refinement can be performed an arbitrary number of times to yield arbitrarily small error. \cite{paper1}. 
...
An extremely important thing to note is that this analysis crucially relies on no low-frequency error being introduced by any subsequent level of SRMG (e.g. via aliasing). This may cause failure to converge not only on this given patch, but on any subsequent subdomain, since my analysis has shown that no method of refinement will eliminate this error on a subpatch once it has been introduced.  
...
The requirement that no low-frequency error be introduced is a cumbersome requirement, which does not bode well for a numerical algorithm. However, the analysis here can be used to guide the requirements on a buffer region for a fixed number of SRMG levels. One or two level SRMG can reduce the memory and communication requirements significantly, allowing textbook multigrid convergence of a 2-4 $\times$ higher-resolution domain than may have been possible with traditional multigrid. 
...
Computational resources and/or a suitably optimized spectral solver were not available to test the newfound convergence requirements. However, we have shown here several numerical experiments and a rigorous analysis of Segmental Refinement. All are consistent with previous work, where the buffer size had to grow to accommodate further levels of SRMG. We have also shown potential solutions for this issue, although the feasibility of these solutions is yet to be determined. 
